...
export const findCommands = async (id: string) => {
  ...
  for (const cmd of cmds) {
    ...
    const cmdProps = {
      name: cmdName,
      description: (module.description as string) ?? ('No description!' as string),
      category: cat ?? 'No category specified!',
      examples: '> ' + file.props.examples?.split(',')?.join('\n> '),
      options: (module.options as SernOptionsData[]) ?? ([] as SernOptionsData[])
    };
...
export const findCategories = (id: string) => {
  ...
  for (const folderPath of folders) {
    ...
    if (
      ...
      (cmdFolder === 'owner' && env.OWNER_ID !== id) ||
