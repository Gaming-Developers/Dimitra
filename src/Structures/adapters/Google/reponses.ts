//This file works but needs to be modified dynamically per rp server and per dept.

import { google } from 'googleapis';
import { GoogleCredentials as credentials } from '#sern/ext';
import { EmbedBuilder, TextChannel } from 'discord.js';
import { Service } from '@sern/handler';
import { Paginator } from '#sern/ext';
type Questions = { id: string; question: string }[];

async function getFormResponses(channel: TextChannel, formId?: string): Promise<EmbedBuilder[]> {
  const prisma = Service('prisma');

  const pages: EmbedBuilder[] = [];

  const main = new EmbedBuilder({
    author: { name: 'New Application Submitted' },
  ...
  if (!formId) formId = '1u3EXWtRCxh2_TkwuoH0_Fo2nBdqa_LbLIDRO_Fpy4Zo';
  ...
  const questionsById: Questions = [
    ...
    {
      id: '47c4f83f',
      question: 'Do you understand that asking about your application before 24 hours could lead to it being denied?'
    }
  ];
  ...
  function createResponseEmbed(response: any, questions: Questions) {
    const firstEmbed = new EmbedBuilder({
      fields: questions
        ...
        .map(question => {
          ...
          const answerValue =
            answer && answer.textAnswers && answer.textAnswers.answers.length > 0
    ...
    const [secondEmbed, thirdEmbed, fourthEmbed] = [buildEmbed(6), buildEmbed(7), buildEmbed(8)];
    ...
    for (const response of responses) {
      if (response) {
        ...
        const modifiedAnswers = Object.values(answers!).map((answer: any) => {
          const question = questionsById.find(q => q.id === answer.questionId);
        ...
        const answersArray = questionsById.map(question => {
          const foundAnswer = modifiedAnswers.find(answer => answer.questionId === question.id);
          return foundAnswer || { question: question.question, value: 'No answer' };
        });
        ...
        const { firstEmbed, secondEmbed, thirdEmbed, fourthEmbed } = createResponseEmbed(response, questionsById);
...
export const fetchNewForms = async () => {
  setInterval(
    async () => {
      const channel = Service('@sern/client').channels.cache.get('1230575538813276160') as TextChannel;
      const embeds = await getFormResponses(channel);
      if (!embeds || embeds.length < 2) return;
      await new Paginator({
        channel,
        content: '<@&1231658494139174912>, someone just filled out a new application!',
        embeds,
        ephemeral: false
      }).run();
    },
