...
export function cooldown(
	...
	usage: Omit<
		...
		"userId" | "actionId" | "guildId" | "channelId"
	>
) {
	async function get(
		interaction:
		...
		if (typeof result === "object") {
			await client.users.cache.get(env.OWNER_IDS[0])?.send({
				content:
					"There was an error implementing cooldowns! " + result.main
			});
	...
	if (commandType === CommandType.Slash) {
		return CommandControlPlugin<CommandType.Slash>(
			async ({ interaction }) => {
	...
	if (commandType === CommandType.MentionableSelect) {
		return CommandControlPlugin<CommandType.MentionableSelect>(
			async (interaction) => {
				return await get(interaction);
			}
		);
	...
	if (commandType === CommandType.StringSelect) {
		return CommandControlPlugin<CommandType.StringSelect>(
			async (interaction) => {
				return await get(interaction);
			}
		);
	...
	if (commandType === CommandType.RoleSelect) {
		return CommandControlPlugin<CommandType.RoleSelect>(
			async (interaction) => {
				return await get(interaction);
			}
		);
	...
	if (commandType === CommandType.UserSelect) {
		return CommandControlPlugin<CommandType.UserSelect>(
			async (interaction) => {
				return await get(interaction);
			}
		);
	...
	if (commandType === CommandType.ChannelSelect) {
		return CommandControlPlugin<CommandType.ChannelSelect>(
			async (interaction) => {
				return await get(interaction);
			}
		);
	...
	if (commandType === CommandType.CtxMsg) {
		...
		return CommandControlPlugin<CommandType.CtxUser>(
			async (interaction) => {
				return await get(interaction);
			}
		);
