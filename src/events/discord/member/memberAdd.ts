...
export default discordEvent({
	...
	async execute(member) {
		...
		//HANDLE REPORTS
		if (
			member.guild.id === "1228509238889021502" &&
			members.some((m) => m.id === member.id)
		) {
			const reports = members.find((m) => m.id === member.id)?.reports!;
			if (reports?.length! > 0) {
				const report_chan = (await member.guild.channels.fetch(
					"1209991920269660170"
				)) as TextChannel;
				await delay(2.5);
				...
				const row = new ActionRowBuilder<StringSelectMenuBuilder>({
					components: [
						new StringSelectMenuBuilder({
							...
							placeholder:
								"Select an option to be added to a report!",
			...
			if (settings && settings.channels) {
				...
				const modChannel = member.guild.channels.cache.get(
					moderatorChan
				) as TextChannel;
				const rulesChannel = member.guild.channels.cache.get(
					rulesChan
				) as TextChannel;
				...
				const verifyButtons = new ActionRowBuilder<ButtonBuilder>({
					components: ["✅|Bypass-Verify", "👢|Kick", "💥|Ban"].map(
						(choice) => {
